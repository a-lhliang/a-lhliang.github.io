document.addEventListener('DOMContentLoaded', function () {
    ...
    const msgToTraditionalChinese = '<i class="lhlfont icon-complex-font"></i><span>轉為繁體</span>' // 此处可以更改为你想要显示的文字
    const msgToSimplifiedChinese = '<i class="lhlfont icon-simple-font"></i><span>转为简体</span>' // 同上，但两处均不建议更改
    ...
    function translatePage() {
        if (targetEncoding === 1) {
            ...
            translateBody()
            if (isSnackbar) pty.snackbarShow(snackbarData.cht_to_chs)
            ...
            saveToLocal.set(targetEncodingCookie, targetEncoding, 2)
            translateBody()
            if (isSnackbar) pty.snackbarShow(snackbarData.chs_to_cht)
        }
        rm.hideRightMenu();
    }
