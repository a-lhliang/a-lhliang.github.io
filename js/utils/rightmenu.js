...
rm.writeClipImg = function (imgsrc) {
    ...
    console.log('触发复制');
    rm.hideRightMenu();
    pty.snackbarShow('正在下载中，请稍后', false, lazyTime)
    if (rm.downloadimging == false) {
      rm.downloadimging = true;
      setTimeout(function () {
        copyImage(imgsrc);
        pty.snackbarShow('复制成功！图片已添加盲水印，请遵守版权协议');
        rm.downloadimging = false;
      }, "${lazyTime}")
    }
}
...
rm.copyUrl = function (id) {
    ...
    let text = id;
    let input = document.getElementById("copyVal");
    input.value = text;
    input.select();
...
rm.copyPageUrl = function () {
    ...
    rm.copyUrl(url);
    pty.snackbarShow('复制本页链接地址成功', false, 2000);
    rm.hideRightMenu();
}
...
rm.searchBaidu = function () {
    pty.snackbarShow('即将跳转到百度搜索', false, 2000);
    setTimeout(function () {
...
function addRightMenuClickEvent() {
    ...
    $('#menu-commentBarrage').on('click', LHL.switchCommentBarrage);
    ...
    $('#menu-downloadimg').on('click', function () {
        LHL.downloadImage(domImgSrc, 'LHL');
    });
    ...
    // 音乐
    $('#menu-music-toggle').on('click', LHL.musicToggle);
    $('#menu-music-back').on('click', LHL.musicSkipBack);
    $('#menu-music-forward').on('click', LHL.musicSkipForward);
    $('#menu-music-copyMusicName').on('click', function () {
        rm.rightmenuCopyText(LHL.musicGetName());
        pty.snackbarShow('复制歌曲名称成功', false, 3000);
    });
}
