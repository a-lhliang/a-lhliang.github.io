window.addEventListener('load', () => {
  const openSearch = () => {
      ...
      pty.animateIn(document.getElementById('search-mask'), 'to_show 0.5s')
      // shortcut: ESC
  ...
  const closeSearch = () => {
      ...
      pty.animateOut(document.getElementById('search-mask'), 'to_hide 0.5s')
  }
  ...
  const searchClickFn = () => {
      ...
      // Respond to Open Search shift+S
      $(window).on('keydown', function (ev) {
          // Escape
          if (ev.keyCode === 83 && ev.shiftKey && LHL_keyboard) {
              if (selectTextNow) {
                  openSearch();
                  let t = document.getElementsByClassName('ais-search-box--input')[0];
                  let evt = document.createEvent('HTMLEvents');
  ...
  const search = instantsearch({
      ...
      searchFunction: function (helper) {
          ...
          if (searchInput.value) {
              let innerLoading = '<i class="lhlfont icon-spinner"></i>';
  ...
  search.addWidget(
      instantsearch.widgets.pagination({
          ...
          showFirstLast: false,
          labels: {
              first: '<i class="lhlfont icon-left-arrow-double"></i>',
              last: '<i class="lhlfont icon-right-arrow-double"></i>',
              previous: '<i class="lhlfont icon-arrow-left-s-line"></i>',
              next: '<i class="lhlfont icon-arrow-right-s-line"></i>'
          },
          cssClasses: {
              root: 'pagination',
